...
<template>
  <section class="page-head" aria-labelledby="plugins-page-title">
    <div>
    ...
    <ul v-if="visiblePlugins.length" class="plugin-grid">
      <li v-for="plugin in visiblePlugins" :key="plugin.id">
        <article
          ...
          role="button"
          tabindex="0"
          :aria-label="`查看 ${plugin.displayName['zh-CN']} 详情`"
          @click="openPlugin(plugin, $event)"
          @keydown.enter="openPlugin(plugin, $event)"
          @keydown.space.prevent="openPlugin(plugin, $event)"
        >
