...
- 人物形象：three.js MMDLoader 加载用户本地 PMX 模型（默认甘雨），立绘仅作加载占位；模型资产因版权规则不随包分发，放于 `~/.dsh/storages/dsh-virtual-companion/models/`。
...
- `pnpm --filter @suhan-dsh/virtual-companion test`
- 根目录 `pnpm check`
- 浏览器人工验证：人物始终置顶、3D MMD 模型渲染（待机呼吸/眨眼/说话口型/表情 VMD）、拖拽移动、单击开始语音、双击打开设置、切换角色/音色/背景信息/实时开关、说话轻轻浮动并恢复、右上角 🤔、模型主动提问、语音回答、连续对话、再次单击停止
