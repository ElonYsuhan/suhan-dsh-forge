...
DeepSeek Harness Web 客户端插件：**始终置顶的透明浮空 3D 卡通仙女**，可拖拽到页面任意位置；单击人物即开始由模型主动提问的语音聊天，双击人物打开设置面板。
...
- **浏览器半**（`src/client/`）
  - `shell.overlay` 条目：始终置顶的全页面透明浮空 3D 卡通仙女（只保留“卡通仙女”一个模型，不持续旋转）
  - 鼠标按住拖动即可移动，单击开始或停止语音聊天，双击打开设置面板
  - 设置面板支持人物角色、人物皮肤、背景信息、女声音色和流式实时回复
  - 聊天时右上角显示 🤔，不显示说话文字；说话时人物放大 1.2 倍，说完恢复
...
- 人物皮肤：仙粉、仙蓝、仙紫、仙绿四套仙女风格外观，可切换裙子与翅膀配色。
...
- 模型跟随 DSH Web 当前选择的模型；3D 外观固定为“卡通仙女”。
...
- `pnpm --filter @suhan-dsh/virtual-companion test`
- 根目录 `pnpm check`
- 浏览器人工验证：人物始终置顶、3D 卡通仙女渲染、不持续旋转、拖拽移动、单击开始语音、双击打开设置、切换角色/皮肤/音色/背景信息/实时开关、说话放大 1.2 倍并恢复、右上角 🤔、模型主动提问、语音回答、连续对话、再次单击停止
