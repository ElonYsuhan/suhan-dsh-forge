...
describe('taskboard execution workflow', () => {
  it('creates a complete Agent, attaches it once, and rejects concurrent execution', async () => {
    ...
    expect(deliveryResponse.body.item.status).toBe('done')
    ...
    expect(firstAdvance).toContain('卡片已推进')
    expect(secondAdvance).toContain('卡片已推进')
    ...
    expect(autoDeliveringItem.status).toBe('in-dev')
    ...
    expect(retryIntegration.body.item.status).toBe('done')
