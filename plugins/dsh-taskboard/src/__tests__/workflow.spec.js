...
describe('taskboard execution workflow', () => {
  it('creates a complete Agent, attaches it once, and rejects concurrent execution', async () => {
    ...
    expect(runResponse.body.item.status).toBe('in-dev')
    ...
    expect(approveResponse.body.item.status).toBe('accept')
    ...
    const secondAdvance = await taskTool.execute({ outcome: 'stage_complete', summary: '排期完成' }, { agent: autoAgent })
    expect(firstAdvance).toContain('卡片保持在本列')
    expect(firstAdvance).toContain('delivery_ready')
    expect(secondAdvance).toContain('卡片保持在本列')
    ...
    await taskTool.execute({ outcome: 'delivery_ready', summary: '交付与验证完成' }, { agent: autoAgent })
    const autoDeliveringResponse = response()
    await route(request('GET', '/taskboard/boards'), autoDeliveringResponse)
    const autoDeliveringItem = autoDeliveringResponse.body.boards['workspace-1'].items.find(item => item.id === autoItemId)
    expect(autoDeliveringItem.status).toBe('accept')
    expect(autoDeliveringItem.executionState).toBe('awaiting-delivery')
    ...
    await vi.waitFor(async () => {
      ...
      await route(request('GET', '/taskboard/boards'), resolvedBoardResponse)
      const resolved = resolvedBoardResponse.body.boards['workspace-1'].items.find(item => item.id === conflictSourceId)
      expect(resolved.archived).toBe(false)
      expect(resolved.status).toBe('accept')
      expect(resolved.integrationState).toBe('merged')
      expect(resolved.commitRef).toBe('resolved-commit')
      expect(resolved.taskWorkspace).toBeUndefined()
    })
