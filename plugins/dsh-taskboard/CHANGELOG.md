...
### Added
- 任务依赖随时可配置：详情页新增「配置依赖」入口（执行开始前均可用，含方案未确认的 AI 工作项与旧数据卡片，不再局限于新建/编辑表单）。保存后按新依赖重新评估执行闸门——前置任务均已交付完成时自动开始执行本任务（无需手动 run，手动执行始终可用）；前置未完成则挂起，全部完成后自动开始；方案未确认的 AI 项等待确认方案后再自动开始。执行中的工作项仍不可配置（409）。
- 详情页原始需求回到方案面板上方，默认只显示三行、末行渐变虚化提示，点击展开/收起全文，长描述不再挤占方案区域。

### Changed
- 任务依赖配置区抽取为共享组件（新建/编辑表单与独立依赖编辑器共用）；无实际变更的保存不触发自动执行评估。

## [0.1.9] - 2026-08-22

### Added
- 新建想法对话框也支持配置任务依赖（与编辑表单共用同一区块）：「保存草稿」和「任务方案生成」均可带上依赖创建，依赖状态在详情页可见。

### Fixed
- 详情页长描述不再挤占核心方案：AI 流程项的方案面板（任务方案生成/确认/冻结方案）调整到原始需求之上优先展示，需求描述限高 220px 内部滚动。

## [0.1.8] - 2026-08-22

### Added
- 新建想法支持「保存草稿」：仅创建卡片、不启动方案生成，之后可随时打开详情生成任务方案；「任务方案生成」保持创建后立即分析的原流程。
- 任务依赖（可选，最多 10 项）：编辑工作项时可关联当前项目下的其他任务并配置类型——在此之前（本任务先执行，完成后自动开始该任务）/ 之后（本任务等待该任务，其完成后自动开始本任务）/ 并行（无顺序约束）。被依赖任务交付并确认提交后，看板自动串行启动后继任务（依赖未满足时确认方案会先挂起，待前置完成后自动开始）；详情页展示依赖条目与语义提示。
- 依赖校验：不能以自身为依赖、不能重复关联、目标必须在本看板中；`dependencies: null` 清空依赖。

### Changed
- 全部原生 DOM 弹窗/选择器迁移到 Radix UI（@radix-ui/react-dialog / select / alert-dialog / toggle-group），此后看板工程统一使用 Radix UI 组件库：工作项编辑/详情/看板设置/确认框/聊天页待办均改为 Radix Dialog/AlertDialog，原生 `<select>` 全部替换为 Radix Select（含看板页与 DSH 设置页的项目切换），方案字段编辑/预览切换改为 ToggleGroup；焦点管理、Esc 关闭、遮罩点击关闭等行为保持一致。
- 移除自实现的 `useDialogFocus` 焦点管理 hook（由 Radix Dialog 内建焦点圈替代）。
