...
- 任务会话绑定到以“原项目 · 任务标题”命名的 DSH Workspace；成功后保留其干净 worktree 和 Workspace，确保历史面板可重新打开完整会话。删除或强制关闭时才分离会话并清理注册与隔离目录。
...
- 任务隔离目录：`$DSH_HOME/storages/dsh-taskboard/worktrees/`（运行时创建；成功任务为可打开的历史会话保留，删除或强制关闭时清理）
